...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const ferrNavItems = [
    { key: 'emprestimo-ferramentas', label: 'Empréstimo Ferramentas', icon: Wrench },
    { key: 'epi-fornecimento', label: 'Fornecimento EPIs', icon: HardHat },
    { key: 'material-consumo', label: 'Materiais Consumo', icon: PackageOpen },
  ];
  ...
  const renderNavSection = (
    ...
    const visibleItems = items.filter(item => hasPermission(item.key));
  ...
  return (
    <aside className="w-16 md:w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col py-4 shrink-0 transition-all duration-200 overflow-y-auto">
      ...
      {renderNavSection(almoxNavItems, 'Almoxarifado', false)}
      {renderNavSection(engNavItems, 'Engenharia & IA', true)}
      {renderNavSection(ferrNavItems, 'Ferramentas & Fornecimento', true)}
