...
import { getSaldo, getStatus, formatCurrency, formatNumber } from '../lib/storage';
import { FAMILIAS } from '../data/mockData';
import { Package, TrendingUp, AlertTriangle, Filter, Plus } from 'lucide-react';
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {filteredItems.filter(i => getStatus(i) !== 'OK').slice(0, 8).map((i, idx) => {
                  const st = getStatus(i);
                  const saldo = getSaldo(i);
                  return (
                    <tr key={`${i.obraId || 0}-${i.codigo}-${idx}`} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
            ...
            <table className="w-full text-xs text-left">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {recentMovements.map((m, idx) => (
                  <tr key={`${m.obraId || 0}-${m.tipo}-${m.id}-${idx}`} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
